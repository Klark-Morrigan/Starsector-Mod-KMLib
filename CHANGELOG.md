...
- `ColonisationRoutines.registerRoutine()`, `OwnershipTransferRoutines.registerRoutine()`, `OwnerSubmarketRules.registerRule()` and `ModdedSystemAccessRoutes.registerRoute()` take a `Supplier<ModIntegration>`: which mod the implementation comes from and what the registering mod loses without it, composed only where the implementation fails. `NexerelinIntegration.installRoutines()` and `RandomAssortmentOfThingsIntegration.installModdedSystemAccessRoutes()` take the same supplier and pass it on, the caller being the mod that loses something.
