...
- **`WiringSteps`**: the guard one step of a mod's start-up wiring runs behind, in `starsector/startup/`. A step that throws costs its own registration rather than every step after it or every mod loading behind it. A step that cannot link what it binds to costs the same: a third party that moved a class, renamed a member or changed a signature raises a `LinkageError` rather than an exception, and raises it where the step first reaches the class, so catching only `RuntimeException` would miss the one failure an integration step is guarded for and let a mod that merely updated take the load down. An exhausted heap or a blown stack still propagates, being the process's trouble rather than the step's. Constructed with the wiring mod's own logger rather than holding one, because a level set through `KmLogging` scopes to a package subtree. One method runs a step that binds to nothing a player could act on; the other takes the integration and reports as well as logs.
