...
A binding to Fast Rendering's bridge that stops holding now costs the map's cursor reading for the session rather than the render pass, and the player is told once, in-game, naming Fast Rendering and both versions. The channel it reports through is generic: any binding to third-party code records into it.

- **Built-against version stamped into the jar**: the build reads the Fast Rendering release the bridge adapter was type-checked against off the bound `fr.jar` and generates it into the jar as a constant, so a mismatch report can name both halves of the disagreement rather than only the half it can still see. A build against the bridge stubs stamps no version.
- **`CompatibilitySubject`** and **`CompatibilityFailure`**: the third party a compatibility report is about, with the version KMLib was compiled against beside the one installed now; and one binding to it that has stopped holding, composing the modal shown to the player and the line written to the log. An unread version renders as an explicit unknown rather than as `null`.
- **`CompatibilityConsumer`**: the mod that took a binding, as the key its records latch under and the sentence naming what it loses. The sentence is the taking mod's rather than the library's: what a failed binding costs is knowledge of the feature built over it, and the library knows the third party, both versions, the member that moved and what was thrown, and nothing about what was drawn over the reading it can no longer serve.
- **`CompatibilityFailures`**: the session's record of those failures, each binding latched on its first record from any thread and held until a reporter takes it. A binding is a third party and the mod that took it, not the third party alone - one broken third party costs every mod bound to it something of its own, so a latch on the third party would keep whichever mod recorded first and leave the rest's players told what another mod lost, or told nothing. The failure is described through a supplier invoked only on the record that is kept, so what it costs to describe one - a reflective probe, a version read - is paid once per binding rather than once per frame; and a take hands over without unlatching, so a binding that fails on every frame is reported on one.
  - **`CompatibilityFailures.SESSION_RECORD`**: the one record every binding records into, held per session rather than per sector because a binding is resolved once for as long as the game runs and can first fail at a load step before any sector exists. `KMLib_ModPlugin` installs the notice that drains it on every game load, as a transient script on the loaded sector.
- **`CompatibilityNotice`**: the per-frame script that drains that record and shows each failure as the game's own message dialog, logging the line a report is written from before the dialog is asked for. One dialog per frame, held while the campaign UI is not up yet or another dialog is, since the game drops a message dialog asked for behind one. Transient: what it reports is a fact about the jars installed this session, not about the save.
- **`UnavailableModelviewMatrixReader`**: the third `ModelviewMatrixReader`, whose every read is no reading. A renderer whose matrix cannot be reached still hands a caller a reader rather than a null one, and one that answers nothing rather than GL's identity, which under a CPU-tracked modelview describes nothing. Not a new caller contract: `CampaignMapTransform` already parks on an absent reading.
- **`FastRenderingBridgeDiagnostic`**: once a binding to Fast Rendering's bridge has failed, which of the six mirrored members no longer hold on the installed jar and why, beside the version the jar reports and the one the build was type-checked against. Probed by name and exact signature through method handles, since the script classloader refuses `java.lang.reflect` to mod code; the version is read under its own guard, so a version that cannot be read costs the version and never the member findings. Never runs on the healthy path.
