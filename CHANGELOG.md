...
- **`CompatibilitySubject`** and **`CompatibilityFailure`**: the third party a report is about with both versions, and one binding to it that stopped holding. An unread version renders as an explicit unknown rather than as `null`. A failure composes two readings for two audiences: the player's modal names the third party, the versions and what stops working, while the log gets a block of labelled rows carrying the mechanics - both versions, which guard caught the binding, what no longer holds, which consumer filed it and what that consumer lost - with the throwable handed to the logger beside it. Its four components are four distinct types, so no two can be transposed. A block rather than a line because a reader comparing two logs needs the values to line up under each other, and because the row that matters varies by report.
- **`CompatibilityBreakage`**: which guard caught a binding and what no longer holds, as one value. Neither diagnoses anything alone - a member that moved and an entry point a release declares and then refuses both arrive naming the same member, and only where each was caught tells them apart - and held together they keep the failure's own components down to four distinct types.
- **`CompatibilityConsumer`**: the mod that took a binding - its mod ID, which of its features the binding serves, and the sentence naming what that feature loses. The latch key is composed from the ID and the feature rather than supplied whole, so two mods cannot spell one key and end up behind a single latch with only the first one's player told anything; the feature half stays the caller's, because one mod can take two bindings to one third party and lose two different things by them. The ID is resolved against the game's mod manager only when a report is composed, so the healthy path reads nothing and an ID naming no installed mod shows as itself. A fourth, optional sentence names what the failure does *not* cost, and is the consuming mod's to write for the same reason the first is: the library cannot vouch for another mod's feature or another mod's save. That sentence is the taking mod's, never the library's.
- **`CompatibilityFailures`**: the session's record, latched per third party and consuming mod, written from any thread and drained by a reporter. The failure is described through a describer invoked only on the record that is kept, so a reflective probe or a version read is paid once per binding rather than once per frame; the describer is handed the consumer to compose against, which is the caller's own except where the record had to tell a reused key apart. Handed over one at a time, since a reporter can only show one at a time, and taken only once a frame can show it - so nothing is ever held outside the record.
  - **A feature key one mod reused reports both features**: the mod half of a key cannot collide between mods, so what is left is one mod spelling one feature key for two features, and latched on the pair alone that dropped the second feature's report exactly as it drops the same feature recording twice. The latch is now the subject, the consumer key and the consumer's sentence: the same feature again carries the same sentence and is dropped, while a second feature under one key is kept under `<feature>-2`, then `-3`, so both reports arrive and the log's consumer row names the reuse where its author will see it. Numbered at the record and never where a consumer is built, because that value is built afresh every time a map's publisher is, and numbering each construction would break the latch and report one failure once a frame forever.
  - **`CompatibilityConsumer.deconflictFeatureKey()`**: the same mod and sentences under the feature key numbered, which is what the record hands a describer in that case.
...
- **`InstalledMods.readModName()`** and **`readModVersion()`**: the display name the game holds for a mod ID and the version that mod declares, or nothing where the game is not up far enough to answer, lists no such mod, or holds a spec stating no version. Guarded like `ModPresence` beside it, a caller asking either being one composing a report - and a report that threw while naming who it was about would lose the report.
- **`WiringSteps`**: the guard one step of a mod's start-up wiring runs behind, in `starsector/startup/`. A step that throws costs its own registration rather than every step after it or every mod loading behind it. Constructed with the wiring mod's own logger rather than holding one, because a level set through `KmLogging` scopes to a package subtree: a library logging a consuming mod's failed step under `kmlib` would put it outside the switch that mod's player turns up. One method runs a step that binds to nothing a player could act on; the other takes the integration and reports as well as logs.
- **`ModIntegration`**: a third-party mod a start-up step binds to and what the wiring mod loses where the step does not take, composing the failure from what was thrown. The version pair runs the other way round from a binding to a renderer patch - nothing was compiled against an optional mod, so the built-for row stands at its unknown wording while the installed one is read off the mod manager - and there is no probe to run, the step having called into the mod and the mod having refused, so what was thrown is the whole of the finding. Supplied to the guard as a supplier rather than a value, so the wording and the version read stay off the load path of every install where nothing broke.
- **`KmlibMod.MOD_ID`**: the library's own ID, held apart from the plugin so a class can say who it belongs to without loading a `BaseModPlugin` subclass for a string. The library files under it as a consumer of its own compatibility channel, a failed integration costing one of its features.
