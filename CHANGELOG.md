...
- **`WiringSteps`**: the guard one step of a mod's start-up wiring runs behind, in `starsector/startup/`. A step that throws costs its own registration rather than every step after it or every mod loading behind it. Constructed with the wiring mod's own logger rather than holding one, because a level set through `KmLogging` scopes to a package subtree. One method runs a step that binds to nothing a player could act on; the other takes the integration and reports as well as logs.
...
- **`ModInfoBase`**: what a fragment reaches of the committed `mod_info.base.json` - the text fields it would fall back to and the dependency IDs it may name. `LocalisationDirectory.readModInfoBase()` reads it from beside the directory, and answers nothing for a mod still committing its launcher file whole.
- **`LocaleParity`**: holds every locale to the default, file by file, each check answering a list of findings worded as the edit to make. Strict where a gap has nothing true behind it: a declared locale with no directory and a directory nobody declared, a mapped file a bundle lacks, a strings key a locale lacks or adds, a slot taking a different argument (reordering by number is allowed), text outside Latin-1 in a locale naming no `coreLocalisation`, a settings row missing, added or out of order, a row varying its type, stored default, options or bounds, a tab split or merged by translation, and a fragment naming a dependency the base does not declare or having no base to merge over. Reporting where it has one: `listModInfoFallbackFieldNames()` lists the launcher fields each locale leaves to the base without failing anything. A Radio's options are held identical because LunaLib stores the label picked, so a translated list would reset the setting for every player moving between builds. Each defect is found once: a missing directory or file is left out of the comparisons over it, and a file the manifest does not map is not compared at all.
- **`LocaleBundle.openSettingsTable()`**: the bundle's settings table, read through `LunaSettingsTable` like the shipped one.
- **`LunaSettingsTable.readBehavioursByFieldId()`, `readDisplayedTexts()` and `readTabsByFieldId()`**: a row split into what it does and what it says. A behaviour is the `FieldBehaviour` of type, stored default, options and bounds, with `describeDifferencesFrom()` naming each differing column as the header spells it; a caption's or a prose row's default is words rather than a value, so it is text. The displayed texts are exactly the cells the screen draws. A field ID declared by two rows fails either keyed reading rather than being dropped from it.
- **`StringTemplates.readArgumentConversions()`**: which argument each slot of a template takes and as what, whatever order the slots are written in - what two wordings of one string must agree on for one call site to fill both. A relative `%<s` reuses the previous slot's argument, and upper-case conversions read as their lower-case forms.
- **`ShippedJson.requireList()`**: narrows a member to a JSON array, beside the object and string checks.
