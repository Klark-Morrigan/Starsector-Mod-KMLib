...
- **A bridge that breaks where it is called degrades too, not only where it is linked**: the deferred copy the Fast Rendering reader enqueues is now total by construction. A command that throws where that renderer runs it is not caught where it was enqueued - the renderer re-throws it wrapped on the game thread at the next frame swap, outside every KM stack frame - so a body that is not total kills the game over a hover highlight, and a guard around the enqueue never sees it. The matrix read is taken inside the guard rather than before it, the bridge member answering it being as able to stop holding as the copy is. What the guard catches latches the reading unavailable for the session, drops the copy taken before the break rather than going on reporting a stale matrix, and records one failure however many frames the map stays open. Both sides of the binding compose their report the same way, so a player is told one thing about one renderer whichever side noticed.
