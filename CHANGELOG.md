...
- **`CompatibilityFailures`**: the session's record of those failures, each subject latched on its first record from any thread and held until a reporter takes it. The failure is described through a supplier invoked only on the record that is kept, so what it costs to describe one - a reflective probe, a version read - is paid once per subject rather than once per frame; and a take hands over without unlatching, so a binding that fails on every frame is reported on one.
