...
| [`map/controls`](map/controls/) | vanilla | the writes into the map's own furniture, as against the reads below it: [`MapFilterRows`](map/controls/MapFilterRows.java) reaches the row of toggles a map screen is furnished from - the `M` screen's full-width strip and the intel visor's shorter band alike, both being the same widget reached by the same unobfuscated accessor off the map itself - and hands it back as a [`MapFilterRow`](map/controls/MapFilterRow.java), a handle rather than the widget, which answers only which row it is. That question is the one a caller cannot answer for itself: the map widget builds its row inside its own constructor, so every open of the screen produces a new one and a control attached to the previous row draws nowhere while the new row stands bare |
