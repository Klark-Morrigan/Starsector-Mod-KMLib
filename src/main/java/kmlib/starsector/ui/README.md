...
| [`ModelviewMatrixReader`](map/transform/ModelviewMatrixReader.java) | the campaign map's transform | [`GlModelviewMatrixReader`](map/transform/GlModelviewMatrixReader.java), [`FastRenderingModelviewMatrixReader`](map/transform/FastRenderingModelviewMatrixReader.java), [`UnavailableModelviewMatrixReader`](map/transform/UnavailableModelviewMatrixReader.java) |
...
| [`widgets/scroll`](widgets/scroll/) | neutral | what a scrolled region is and what its bar looks like over it: [`ScrollRegion`](widgets/scroll/ScrollRegion.java) - a viewport, how far its content overruns, and the container whose right gutter the bar stands in - with [`ScrollState`](widgets/scroll/ScrollState.java) holding the raw position request a wheel or a drag nudges, clamped each frame by whoever knows the real overflow. [`Scrollbar`](widgets/scroll/Scrollbar.java) computes the track, thumb, grab column and pointer-to-offset over one of those, and [`PanelScrollbars`](widgets/scroll/PanelScrollbars.java) projects a panel onto it so a paint pass and an input listener read one geometry rather than each re-projecting the placement. How thick the bar draws is an input rather than a width the geometry holds: [`ScrollbarThickness`](widgets/scroll/ScrollbarThickness.java) is the track's width with the two fixed gaps flanking it, and states the gutter a container has to keep clear for it - reserving that gutter belongs to whatever lays the container out, which is the one thing this package will not reach for. A width of nothing takes the bar away rather than drawing one of no width: no track, no thumb, and no grab column claimed, so a press in that inset falls through to whatever is under it. That is one reading on the [placement](widgets/PanelPlacement.java) rather than a test at each pass, since the pass that draws the bar and the pass that grabs its thumb cannot be allowed to disagree about whether there is one - and it is a reading apart from whether the list overruns at all, which stays the wheel's question: the wheel is how a player with the bar off moves the list, so it must not go with it |
| [`layout`](layout/) | neutral | box sizing and placement, strips, padding, and the [tabs row](layout/TabsControlLayout.java) - the one control whose dimensions come from the vanilla tab strip rather than from a body-font label. It is also where the gutter [`ScrollbarThickness`](widgets/scroll/ScrollbarThickness.java) states is actually spent: [`CappedStripLayout`](layout/CappedStripLayout.java) knows what its own padding already holds clear, so a bar no fatter than that costs the body nothing and a fatter one widens the body - and the box framed around it - by the excess, leaving the rows at the width they measured to. Where that body may stand travels as one [`BodyRoom`](layout/BodyRoom.java) - its content anchor and its height limit - built only off the anchor a panel already resolved, so the two coordinates and the limit are never three loose floats in a row for a caller to transpose |
| [`label`](label/) | neutral | label fitting, plus the length-estimator port |
| [`buttons`](buttons/) | vanilla | the words on a button the game built, for a caller decorating a widget it did not draw: [`VanillaButtonLabel`](buttons/VanillaButtonLabel.java) finds them and says a bound key in them. Needed because the published text accessors on a button reach the words of one of the several kinds of button the game builds and quietly do nothing for the rest - reading back null and writing nowhere, through the interface that looks like it should work - and that same split is why the game's own key announcement never reaches those other kinds, leaving a button bound to a key it never mentions. What the words *are* reachable through is the published label the engine draws them with, found by the two accessors the game leaves unobfuscated and by following the hop between them on what it actually answers rather than on what it promises to return, the game declaring that hop as something broader than the piece it hands over and testing the answer itself. Only a label already reading what the button was built with is accepted: a widget holds several - chrome, a tooltip's own, an empty one waiting to be filled - so a search taking the first it met would write into something nobody draws and report nothing. What it says there is the game's own rule for a key, the name lit where the words already hold it and spelled out after them where they do not, so a decorated button reads like the ones beside it. Not finding the words costs what was going to be said and nothing else |
...
| [`render/gl/controls`](render/gl/controls/) | GL | [`ControlRenderer`](render/gl/controls/ControlRenderer.java) and the per-kind widget passes it composes - tick box, radio row and grid, toggle, divider, and the segmented-row chrome a radio and a tab strip share. Every word any of them shows is set by [`ControlLabelRenderer`](render/gl/controls/ControlLabelRenderer.java) instead: what a widget looks like follows the control's kind, while how its words are measured and anchored follows the face and the row, and all of them share that half. It is also where a motion's progress and the shade it travels toward are bound into finished paint per cell - a [wash](render/gl/controls/CellHoverWashSource.java) for the pointer and a [light](render/gl/controls/CellPressLightSource.java) for a press, handed to a widget [as one value](render/gl/controls/CellPaintSources.java) and drawn in that order over the cell and under its marks - so no widget pass reads a cursor, holds a timing, or learns that a fade exists |
...
| [`input`](input/) | GL | the pointer's side of a panel and what the panel holds in answer across frames: [`UiCursor`](input/UiCursor.java) with its [port](input/CursorPosition.java), the [panel](input/PanelController.java) and [tab panel](input/TabPanelController.java) controllers, the [list's own end](input/PanelScrollController.java) holding the wheel, the drag and where the list stands, the one [hit resolver](input/ControlHitResolver.java) a hover and a press both read with the [firing path](input/ControlActivation.java) kept downstream of it, the fades, press lifts and [arrivals](input/HoverArrival.java) a body cell carries - keyed by the place under the pointer rather than by the widget standing in it - the [report](input/BodyHoverReporter.java) back out to the host that built that control, the [sounds](input/PanelSounds.java) a detected moment is answered with, and [how an event is claimed](input/PointerParking.java): a press or a wheel consumed, a move parked instead so the screen behind hears the pointer leave what it had lit. Mechanics and rationale in [its own README](input/README.md) |
| [`highlight`](highlight/) | vanilla | `Highlight`, `HighlightedParagraph`, `HighlightedMessage` |
| [`tooltip`](tooltip/) | vanilla | [`Tooltips`](tooltip/Tooltips.java), the `TooltipCreator` boilerplate wrapper - the vanilla surface's answer to what [`widgets/tooltip`](widgets/tooltip/) holds neutrally. In two forms, because the engine only offers the attachment on a tooltip surface: a caller building its own panel already holds one and hands it over, while a caller decorating a widget the game built holds none and nothing about the target yields one - so the second form makes a panel and a single element, uses them for the call, and drops both. Neither is added to anything and neither draws; what the call leaves behind is on the target component alone |
| [`intel`](intel/) | split | the screen-view port and its vanilla implementation |
| [`coreui`](coreui/) | split | [`CoreUiTree`](coreui/CoreUiTree.java), the by-name reach into the live widget tree that every screen's probes walk; beside it [`CoreUiMethods`](coreui/CoreUiMethods.java), the by-shape reach for the members a name cannot find - what the tree walks are hops the core UI publishes and keeps, while an obfuscated member is renamed with every game build, so a caller after one of those is handed the whole set a class declares and recognises what it wants by the signature, as one [`CoreUiMethod`](coreui/CoreUiMethod.java) it can call without ever naming it. Declared and inherited are two reads rather than one because the difference decides matches: a signature matched over what a class merely inherits meets every base class's members and finds candidates that were never candidates. Both live here for the same reason, this package being the only one in the library that reflects at all - the facility they reach through sitting beside them, split so that the part which can be read is not the part which can be misused: [`ReflectionBypass`](coreui/ReflectionBypass.java) holds the classloader route and the method handles over it and no logic whatever, [`ReflectedMembers`](coreui/ReflectedMembers.java) holds every search and every by-name read, write and call and no way into reflection of its own, and [`ParameterCompatibility`](coreui/ParameterCompatibility.java) holds the one rule deciding which member a caller's arguments reach, touching no reflection at all and so testable on its own. A member any of them finds comes back as one [`ReflectedField`](coreui/ReflectedField.java), [`ReflectedMethod`](coreui/ReflectedMethod.java) or [`ReflectedConstructor`](coreui/ReflectedConstructor.java) - described once and callable without its own type ever being named, those types being exactly what the game's classloader denies mod code. What a search matches on is stated as a [`FieldQuery`](coreui/FieldQuery.java), [`MethodQuery`](coreui/MethodQuery.java) or [`ConstructorQuery`](coreui/ConstructorQuery.java), an unstated criterion meaning "any", which is what lets one call site ask by name where the game publishes a member and by signature where it renames one every build. All of it is package-private, and carried rather than depended on because it is another author's code under a licence of its own - see [THIRD-PARTY-NOTICES.md](../../../../../../THIRD-PARTY-NOTICES.md); the [repaint port](coreui/CoreUiComponentRepainter.java) with its [reflective implementation](coreui/ReflectiveCoreUiComponentRepainter.java), which drives a component's own draw through that same reach, clipped to a region a caller hands it - the one thing here that writes to the tree rather than reading it; and [`CampaignScreenView`](coreui/CampaignScreenView.java), which answers the coarse questions about the core UI without walking into any one screen: which core tab is up, and whether *none* of it is and the player is looking at the campaign world itself (that second one being the first plus "no interaction dialog"). The tab is published API with one correction, and the correction is why the read exists rather than each caller asking the campaign UI: a dialog goes on handing out the core UI of a screen the player has closed, still naming the tab it was showing, so a tab it reported stands only while that core is still on screen - the one thing here that reaches the tree, and only far enough to ask whether a panel is still drawn. Beside it [`CoreUiDialogView`](coreui/CoreUiDialogView.java) answers what stands *over* a screen rather than which screen is up: whether a core screen has raised a modal in front of itself, and - as [`OverlayPresence`](coreui/OverlayPresence.java), off one walk - how far through its own fade that modal stands, which is the same curve it darkens the screen by. Presence and fade travel together because they are owed at different moments and must not disagree: a modal claims input from the frame it is raised, while its fade is still at nothing, so anything standing aside stands aside at once and only what it *paints* follows the fade. That shape is not the modal's alone: anything raised over a screen answers in it, a panel a mod stands in the core UI included - which no walk here can find, descending from nothing of the game's - so a caller stands aside for every such thing on one reading rather than one per kind. Which frames the flag covers is each answerer's to state, and they differ: the game holds a modal raised until its fade has run out, that being how long it keeps it in the tree intercepting, while something a mod raises may hand the screen back on the press and go on painting for the length of its fall. Its own read rather than a corner of the tab one, because the published dialog state answers neither - that reports the campaign's conversations, and a modal stood up inside the core UI is not one - and because anything drawn after the core UI has to ask it before drawing at all. [`CodexView`](coreui/CodexView.java) answers that same question for the codex, and is a separate reading rather than a case of the modal one because the codex is not core UI at all: the campaign state holds a second screen panel of its own for it, so no walk of the core UI's children can reach it and it carries none of the modal marker either. It is read off the app state instead, which reports the codex up both ways it can be raised - over a screen, and from the campaign itself - where a widget-shaped rule would have to find two different things in two different trees |
...
| [`map/transform`](map/transform/) | split | the modelview-matrix port and its three bindings - GL's own, Fast Rendering's deferred one, and the [unavailable](map/transform/UnavailableModelviewMatrixReader.java) one that reports no matrix where neither can be reached - with the [selector](map/transform/ModelviewMatrixReaders.java) between them. That binding is guarded wherever it can stop holding - as it is linked, as the game thread calls it, and as the renderer's own thread runs the command it enqueued - so a bridge which stopped holding costs the reading rather than the render pass it was taken in, and the player is told once with the renderer named rather than the mod that was reading it. Over that sit the [transform](map/transform/CampaignMapTransform.java) inverting the map's pan and zoom and the [cursor read](map/transform/MapCursor.java) that answers what a pointer is over. Mechanics and rationale in [its own README](map/transform/README.md) |
...
| [`map/icons`](map/icons/) | vanilla | the one write into the map widget's draw order: [`MapIconReseater`](map/icons/MapIconReseater.java), the script that takes a caller's entity out of its location until the widget has dropped its icon, so it re-enters at the tail, over the [rule](map/icons/MapIconReseatDecision.java) that says when one is owed - stated over where the icon sits rather than over a map having opened, so a re-seeding nobody witnessed corrects itself, and over the icon having been dropped rather than an advance having passed, since the campaign advances several times per rendered frame under its speed-up and a put-back landing in the removal's own frame lifts nothing. Which entity, which maps matter, and where an icon currently sits are all ports: a rule about the first two would be a guess about somebody's content, and reading the third itself would make this package depend on the probes. The placement port is asked *about* an entity rather than standing for one, so it cannot be wired to a different entity from the one moved. It says in the log what it saw, because a layering that degrades over a session cannot be diagnosed from the picture: each move, and the map coming and going with the count of lifts the icon has not been seen clear since, at DEBUG on the library's own logger; and once per session at WARN, since a failure that does not heal recurs on every open and the first line says all of it: a map that stays up with no icon placeable for the entity - the map read and the placement read disagreeing - and the stand-down, with the [readings](map/icons/ReseatReading.java) that led to it. The stand-down itself is the open's, not the session's: the next open is a fresh widget with a fresh seeding and is owed a fresh try |
