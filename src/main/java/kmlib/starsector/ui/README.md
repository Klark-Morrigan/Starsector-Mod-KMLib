...
| [`screen`](screen/) | vanilla | how big the window is: [`VanillaScreen`](screen/VanillaScreen.java) answers the whole screen as a box, and each axis on its own, in each of the two spaces a UI pass straddles - the UI units a widget is laid out in, and the framebuffer pixels the mouse and a GL clip are stated in. One home for both pairs because they coincide only at a pixel scale of 1, so a number taken from the wrong one is right on the machine it was written on and wrong on a scaled display; the methods are named for their space rather than for the settings call behind them, which is what leaves a mismatched pair unable to read as a matched one. Read afresh at every ask, a window being resizable mid-session |
