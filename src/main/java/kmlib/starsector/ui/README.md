...
| [`sound`](sound/) | split | the [roles](sound/StarsectorUiSound.java) a KM control answers with, the [scheme](sound/UiSoundScheme.java) saying which role each moment makes - carried in the panel's [`WidgetStyle`](render/gl/style/WidgetStyle.java), a sound being a property of a look exactly as a fill is - and the player port with its live binding. The choice belongs to the look and the moment to whatever detects it, so no controller names a sound of its own and a panel is silenced by the one value it was built from |
...
| [`coreui`](coreui/) | vanilla | [`CoreUiTree`](coreui/CoreUiTree.java), the by-name reach into the live widget tree that every screen's probes walk, and [`CoreUiComponentRenderer`](coreui/CoreUiComponentRenderer.java), which drives a component's own draw through that same reach, clipped to a region a caller hands it - the one thing here that writes to the tree rather than reading it |
