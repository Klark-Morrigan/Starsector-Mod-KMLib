...
| [`map`](map/) | split | the transform port, its two implementations, the [cursor read](map/MapCursor.java) over them, the campaign map's [view state](map/CampaignMapView.java) and the [starscape presence](map/StarscapeMapPresence.java) that folds it together with the intel screen's own map, and the two core-UI probes over [`CoreUiTree`](map/CoreUiTree.java)'s by-name reach - [`VanillaMapTooltip`](map/VanillaMapTooltip.java), and the [widget trace](map/MapTabWidgetTrace.java) that describes what the cursor is inside for a consumer to log as its own |
