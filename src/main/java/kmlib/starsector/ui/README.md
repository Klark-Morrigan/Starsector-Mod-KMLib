...
| [`map/probes`](map/probes/) | vanilla | the live reads into the map's widget tree over [`CoreUiTree`](coreui/CoreUiTree.java)'s by-name reach: [`ShownMapTab`](map/probes/ShownMapTab.java), the [surface bounds](map/probes/MapSurfaceBounds.java) that pick the map out of that tab by shape as a [surface area](map/probes/MapSurfaceArea.java) carrying the chrome drawn with it, the [finder](map/probes/EmbeddedMapFinder.java) that answers where the maps which are *not* that tab stand, as [one value per map](map/probes/EmbeddedMap.java) carrying the chain it hangs under and answering, live at each ask, the box it is currently drawn in - never kept, a panel a mod slides on and off screen having moved by the time a held one is read - and, for a caller whose subject is the widget rather than what shows of it, that map as a component, which answers where the box read does not since a map drawn to nothing is a widget still, [`VanillaMapTooltipProbe`](map/probes/VanillaMapTooltipProbe.java) that hands the shown tooltip back rather than a yes/no, so a consumer can step aside for it or draw over it, the [widget trace](map/probes/MapTabWidgetTrace.java) that describes what the cursor is inside, the [host trace](map/probes/EmbeddedMapHostTrace.java) that names whoever owns an embedded map off that same finder's walk, the [icon-order trace](map/probes/MapIconOrderTrace.java) that describes which terrain the widget paints over which - all three handing the line back for a consumer to log as its own - and the [layering probe](map/probes/MapIconLayeringProbe.java) that answers the same subject for one entity as a value to act on. Both icon reads reach the icon map through one shared [reader](map/probes/MapWidgetIcons.java), so the walk and the one warning about it stopping working exist once |
| [`map/suppression`](map/suppression/) | vanilla | the write that stops a map surface drawing at all: [`OffScreenMapSuppressor`](map/suppression/OffScreenMapSuppressor.java), the script that holds a map widget's own opacity at zero while its owner has it parked clear of the screen and hands back the value it was found at when it returns. A widget drawn to nothing renders no subtree, so the map inside it iterates no terrain and contributes no transform to a frame another map is drawn in - which is what a hidden surface otherwise costs a caller that has to resolve a cursor through one of them. Which widget it may be pointed at is a port, writing into somebody else's panel being a decision only that caller can take, and the [screen](map/suppression/VanillaScreenBox.java) the widget's box is compared against is read live so a resized window moves both sides of the comparison. The reading is where the widget *is* rather than what shows of it, the two parting the moment this writes: a read that sifted out widgets drawn to nothing would stop reporting the very map it had just switched off, leaving it parked for good |

The map is five packages rather than one. Three of them read different things - GL state, the
campaign's persisted UI data, the live widget tree - and the other two read none of that: they
act, one moving an entity so the widget seeds its icon later, the other writing a map widget's
opacity so it stops drawing while nobody can see it. **No class in any of them references
...
[`MapIconLayering`](map/MapIconLayering.java) sits above them rather than in one of them, and
...
touches it, which is a contract worth keeping literal. `map/suppression` is apart from the probes
for that same reason, and takes the widget it writes to as a port rather than finding one. Those reads are also the only way to check
