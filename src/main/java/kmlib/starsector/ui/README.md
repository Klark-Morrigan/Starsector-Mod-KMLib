...
| [`widgets/scroll`](widgets/scroll/) | neutral | what a scrolled region is and what its bar looks like over it: [`ScrollRegion`](widgets/scroll/ScrollRegion.java) - a viewport, how far its content overruns, and the container whose right gutter the bar stands in - with [`ScrollState`](widgets/scroll/ScrollState.java) holding the raw position request a wheel or a drag nudges, clamped each frame by whoever knows the real overflow. [`Scrollbar`](widgets/scroll/Scrollbar.java) computes the track, thumb, grab column and pointer-to-offset over one of those, and [`PanelScrollbars`](widgets/scroll/PanelScrollbars.java) projects a panel onto it so a paint pass and an input listener read one geometry rather than each re-projecting the placement. How thick the bar draws is an input, not a constant: [`ScrollbarThickness`](widgets/scroll/ScrollbarThickness.java) carries the track's width along with the two fixed gaps flanking it - the margin off the container's edge, the clearance off the content - so a thicker bar grows leftward from a fixed right edge and the spacing stays what it is while the one dimension a player can judge varies. A type rather than a bare float because it travels beside the offset and overflow a region is otherwise described by, which are same-typed and positional, and a track sized from a scroll offset would compile and paint. What it states is the width the bar needs clear of the content and no more - whether a container's own padding already covers that is arithmetic for whatever lays the container out, and reaching for it here would point these widgets at the layout tier that arranges them |
