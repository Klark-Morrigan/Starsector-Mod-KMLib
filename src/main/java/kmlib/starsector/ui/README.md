...
| [`render/gl`](render/gl/) | GL | every raw-GL painter, its styles, and the [state guard](render/gl/GlStateGuard.java). A tab row is the one control drawn two ways: the [`TabChrome`](widgets/tabs/style/TabChrome.java) its style names is resolved through [`TabChromeRenderer`](render/gl/tabs/TabChromeRenderer.java) to the sector map's [seamless strip](render/gl/tabs/VanillaTabStripRenderer.java) or the intel screen's [raised buttons](render/gl/tabs/RaisedButtonTabStripRenderer.java), so a panel matches whichever tab convention stands nearest it. The enum is substrate-independent and cannot name a GL pass, which is why the binding sits on that seam rather than on the enum - and why nothing that draws a control branches on a chrome. Both read the same laid geometry and the same resolved look and lift - a tab fades, pulses, and blinks alike under either - and both spell a tab out through one [label pass](render/gl/tabs/TabLabelRenderer.java), so only the surface under the shades differs |
