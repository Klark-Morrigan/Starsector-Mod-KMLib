...
| [`map/transform`](map/transform/) | split | the modelview-matrix port, its GL and Fast Rendering implementations, the [unavailable](map/transform/UnavailableModelviewMatrixReader.java) one that reports no matrix at all - a renderer whose matrix cannot be reached still hands a caller a reader, and one that answers nothing rather than GL's identity, which under a CPU-tracked modelview describes nothing - the [selector](map/transform/ModelviewMatrixReaders.java) between them, and the [transform](map/transform/CampaignMapTransform.java) and [cursor read](map/transform/MapCursor.java) built over it |
