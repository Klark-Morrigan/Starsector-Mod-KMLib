...
| [`map/transform`](map/transform/) | split | the modelview-matrix port, its GL and Fast Rendering implementations, the [unavailable](map/transform/UnavailableModelviewMatrixReader.java) one that reports no matrix where neither can be reached, the [selector](map/transform/ModelviewMatrixReaders.java) between them - which takes the Fast Rendering binding under guard, a bridge member that moved costing the reading rather than the render pass it was taken in: the selection degrades onto the unavailable reader and records the failure, so the player is told once and the renderer is named rather than the mod that was reading it - the same guard where that bridge is called rather than linked, the Fast Rendering reader's deferred copy being [held apart](map/transform/FastRenderingModelviewCopy.java) from the binding and total by construction, since a throw on the renderer's own thread is re-thrown a frame later on the game thread with nothing of ours on the stack to catch it: what it catches latches the reading unavailable for the session and records it there, which is also why that reader is built per consumer rather than shared, a reader that cannot say who it serves having nobody to record against. Both sides compose their report through one [holder](map/transform/FastRenderingBridgeFailures.java), so a player hears one thing about one renderer whichever side of the binding noticed - and the [transform](map/transform/CampaignMapTransform.java) and [cursor read](map/transform/MapCursor.java) built over it |
