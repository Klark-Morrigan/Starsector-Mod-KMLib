...
| [`coreui`](coreui/) | vanilla | [`CoreUiTree`](coreui/CoreUiTree.java), the by-name reach into the live widget tree that every screen's probes walk, and [`CoreUiComponentRenderer`](coreui/CoreUiComponentRenderer.java), which drives one component's own draw again through that reach, clipped to a region - what puts a widget the core raised underneath a later pass back on top of it, with only the hidden part repainted so nothing composites twice |
