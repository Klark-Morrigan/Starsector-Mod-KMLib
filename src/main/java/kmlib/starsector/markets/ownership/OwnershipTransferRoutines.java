 ...
 * sequence this library composes. A routine that fails is taken out for the session and reported
 * under the integration that registered it; the hand-over in hand goes to this library's sequence
 * only where the routine failed before doing any of it - see {@link ExtensionPoint#offerWork}.
 *
...
public final class OwnershipTransferRoutines {
    ...
    static void registerRoutine(
        ...
        INSTALLED_ROUTINE.registerImplementation(
            ...
            ownershipTransferRoutine,
            fallbackToDefaults,
            routineFailure -> failureReporter.recordFailure(WHILE_HANDING_A_COLONY_OVER, routineFailure));
    }
