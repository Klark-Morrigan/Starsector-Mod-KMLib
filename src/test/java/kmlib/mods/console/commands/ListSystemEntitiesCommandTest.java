...
final class ListSystemEntitiesCommandTest {
    ...
    private static StarSystemAPI buildSystem(
            ...
            List<SectorEntityToken> allEntities,
        ...
        when(systemMock.getFleets())
            .thenReturn(fleets);

        return systemMock;
    }

    private static SectorEntityToken buildEntity(
            String id,
            String name,
            float x,
            float y,
        ...
        when(entityMock.getName())
            .thenReturn(name);
        when(entityMock.getLocation())
            .thenReturn(new Vector2f(x, y));
        when(entityMock.getOrbitFocus())
            .thenReturn(focus);
        when(entityMock.hasTag(Tags.GATE))
            .thenReturn(isGate);
    ...
    private static CampaignFleetAPI buildFleet(String name, float x, float y) {
        ...
        when(fleetMock.getId())
            .thenReturn(name);
        when(fleetMock.getName())
            .thenReturn(name);
