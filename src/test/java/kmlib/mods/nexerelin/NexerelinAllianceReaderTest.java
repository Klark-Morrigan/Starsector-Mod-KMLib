 ...
 * record rather than on the alliance, so the ID every member shares, the label the alliance carries
 * and the member a reader ranks first are all read out of one row here. A field taken from the
 * wrong place would show as an alliance named after its neighbour, or ranked behind a lesser
 * member.
 *
 ...
 */
final class NexerelinAllianceReaderTest {
    ...
    // order through untouched, and element 0 is the member a reader treats as dominant.
    ...
    class FlattenAlliances {
        ...
        @Test
        void readsAnAlliancesIdNameAndRankedMembers() {

            var alliance = buildAllianceNamed(ALLIANCE_NAME);

            var allianceRecords = NexerelinAllianceReader.flattenAlliances(List.of(alliance));

            assertThat(allianceRecords)
                .hasSize(1);
            ...
            assertThat(allianceRecords.get(0).allianceId())
                .isEqualTo(alliance.uuId);
            assertThat(allianceRecords.get(0).name())
                .isEqualTo(ALLIANCE_NAME);
            assertThat(allianceRecords.get(0).membersSortedDescending())
                .containsExactly(DOMINANT_MEMBER, LESSER_MEMBER);
        }

        @Test
        void yieldsOneRecordPerAllianceInTheOrderGiven() {
            // A caller listing alliances builds its rows off these in order, so a flattening that
            // reordered them would reorder that listing for a reason nothing states.
            var allianceRecords = NexerelinAllianceReader.flattenAlliances(List.of(
        ...
        @Test
        void yieldsNothingForNoAlliances() {
            ...
            assertThat(NexerelinAllianceReader.flattenAlliances(List.of()))
                .isEmpty();
        }
    }
