...
/**
 * Pins the direction's three surfaces: the frozen persistence keys a consumer's save round-trips
 * through, the fallback an absent or unrecognised key resolves to, and the flip a re-pick applies.
 * The keys are pinned as literals so a rename that would silently reset every save's stored direction
