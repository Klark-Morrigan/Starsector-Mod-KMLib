 ...
 * merely believed absent - which is what the stated-renderer readings below are for.
 */
class MapCursorReadTest {
    ...
    class DescribeRead {
        ...
        void describeReadWordsThePixelTheSnapshotAndThePointItLandedOn() {
            ...
            //
            // Nothing rewrote this test's class references, so the renderer this resolves is stock
            // LWJGL and the clip is read - which is what pins that the live read is reached at all.
            stubScissorTestOff();
    ...
    class DescribeReadUnderRenderer {
        ...
        void describeReadUnderRendererTouchesNoGlStateUnderFastRendering() {
            ...
            buildRead().describeReadUnderRenderer(UNDER_FAST_RENDERING);

            glMock.verify(() -> GL11.glIsEnabled(GL11.GL_SCISSOR_TEST), never());
            glMock.verify(
                () -> GL11.glGetInteger(eq(GL11.GL_SCISSOR_BOX), any(IntBuffer.class)),
                never());
        }
    }
