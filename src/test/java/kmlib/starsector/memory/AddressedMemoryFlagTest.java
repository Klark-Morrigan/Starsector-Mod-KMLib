...
import kmlib.testfixtures.starsector.memory.MemoryKeyAddresses;
...
final class AddressedMemoryFlagTest {
    ...
    // That key under the two stand-in addresses, spelled out: what fails if the segment stops
    // being appended, or is appended in a different shape.
    private static final String KEY = "$sample_flag_test";
    private static final String OTHER_KEY = "$sample_flag_other";

    private static final MemoryKeyAddress ADDRESS = MemoryKeyAddresses.createStandInAddress();

    private static final MemoryKeyAddress OTHER_ADDRESS =
        MemoryKeyAddresses.createOtherStandInAddress();
