...
final class EntitySpawnerTest {
    ...
    class SpawnOrbitingCustomEntity {
        ...
        void addsTheEntityToTheFocusLocationAndPutsItOnTheOrbit() {
            ...
            var placement = OrbitPlacement.createOrbiting(100f, 2f, 45f);

            var result = EntitySpawner.spawnOrbitingCustomEntity(
                focusMock,
                "inactive_gate",
                Factions.NEUTRAL,
                placement);
    ...
    class SpawnOrbitingJumpPoint {
        ...
        void buildsPlacesAndWiresHyperspaceWhenTheFocusIsInAStarSystem() {
            ...
            when(factoryMock.createJumpPoint(null, "Corvus Jump-point"))
                .thenReturn(jumpPointMock);

            var placement = OrbitPlacement.createOrbiting(200f, 3f, 90f);

            var result = EntitySpawner.spawnOrbitingJumpPoint(
                focusMock,
                "Corvus Jump-point",
                placement);

            assertThat(result)
                .isSameAs(jumpPointMock);

            verify(jumpPointMock)
                .setStandardWormholeToHyperspaceVisual();
            verify(systemMock)
                .addEntity(jumpPointMock);
            ...
            verify(systemMock)
                .removeTag(Tags.SYSTEM_CUT_OFF_FROM_HYPER);
            verify(systemMock)
                .updateAllOrbits();
        }
        ...
        void skipsHyperspaceWiringWhenTheFocusIsNotInAStarSystem() {
            ...
            when(factoryMock.createJumpPoint(null, "Lone Jump-point"))
                .thenReturn(jumpPointMock);

            var placement = OrbitPlacement.createOrbiting(50f, 1f, 0f);
