...
and primarily serves other KM mod series members
(e.g. [KMU](https://github.com/Klark-Morrigan/Starsector-Mod-KMU)).
...
and a mod plugin that binds the settings and registers its
[optional-mod adapters](#optional-mod-seams)
at application load.
...
- **Random Assortment of Things** (RAT)
  - KMLib recognises Abyssal Fractures,
    and counts them as valid access points into attached systems;
...
- `kmlib.version.template` -
  ...
  Generated output,
  never committed -
  see [Build & Test](#build--test) and [Reusable CI / release actions](#reusable-ci--release-actions).
...
- [`src/testFixtures/java/`](src/testFixtures/java/) -
  ...
  See [Test fixtures](#test-fixtures).
- [`src/test/java/`](src/test/java/) -
  JUnit 5 and Mockito suites,
  and the fixtures only they use.
- [`src/buildScriptTest/java/`](src/buildScriptTest/java/) -
  TestKit suites over the scripts in [`gradle/`](gradle/),
  run by `testBuildScripts`.
...
- [`gradle/starsector-install-locator.gradle`](gradle/starsector-install-locator.gradle) -
  ...
  the install root,
  a core jar by name or by pattern,
  an installed mod's jar,
  and this checkout.
...
Local runners in [`scripts/`](scripts/),
all with a `.sh` and a `.bat` face:

- `run-ci-yaml-and-bash` -
  MAIN entry:
  the lint suite and bats tests in one go,
  needing Git Bash and Docker.
  See [Local linting](#local-linting).
- `run-lint-yaml-and-bash` -
  the lint half only,
  no bats.
- `run-tests-bash` -
  the bats tests only.
- `run-tests-gradle.bat` / `run-coverage-gradle.bat` -
